#Day093 - k-fold cross validation
import pandas as pd
import numpy as np
...
sc = StandardScaler() #feature scaling - age and salary are very low and high
...
knn = KNeighborsClassifier(n_neighbors = 5, p = 2) #Euclidean distance
knn.fit(features_train, labels_train)

#Prediction
prediction = knn.predict(features_test)
len(prediction)
print(list(zip(prediction, labels_test)))
...
k_fold_scores = cross_val_score(estimator = knn, X = features_train, y = labels_train, cv = 10)

print ("Accuracies are ", k_fold_scores)
print ("Mean accuracy is",k_fold_scores.mean())
print ("standard deviation for the accuracies is",k_fold_scores.std())
...
traininig data = 280   (features_train)
testing data   = 120   (features_test)
...
How to generate the dense data set, since we dont have that much amount of data.
...
#If v open and check, there is a pattern, x axis values are changing since y = 0
print(xx)

#If v open and check, there is a pattern, y axis values are changing since x = 0
print(yy)

#We need to take x values from first row of xx and y values from the first row of yy 
#These two will make the points.
#We want to predict for each points generated for drawing the decission boundary.
#For prediction method we need to pass them as one set
...
#To draw the decission boundary need to get the prediction whether it belongs to which category
#xx, yy and Z should have the same shape, so we have to reshape it
Z = knn.predict(pt)
print(Z.shape)

#We have to reshape Z same as xx and yy 
...
#We can use 2 colors and show one set of xx and yy in GREEN which has Z = 0 and in RED which has Z = 1
...
"""
A contour plot is a graph that you can use to explore the potential relationship 
between three variables. Contour plots display the 3-dimensional relationship in 
two dimensions, with x-factors and y-factors (predictors) plotted on the x-scales 
and y-scales and response values represented by contours.
""" 
"""
#plot the decission boundary
plt.contourf(xx, yy, Z, alpha=1.0)

#We have to overlays the points
#Plot the points
...
#We can see from the visual and compare it with cm, that 4 points were different and similarly for 3 points 
plt.show()
"""

cs = plt.contourf(xx, yy, Z, alpha=1.0, extend='both')
