...
import pandas as pd     
#__ called as dunder function  (pd.__version__ : tells version of pandas)
...
type(dt) #DataFrame - only in pandas
#typing dt in console will print the entire dataset.

dt.shape #shape - for row and column numbers

dt.head() #will give first five datas

dt.head(3) #providing a number inside it will print data till that row.

dt.tail() #last five rows datas

dt.tail(6) #providing a number inside it will print data till that row from last row.

dt.sample() #picks random records

print(dt.sample(4)) #4 random records

n = int(input('Enter a number: '))
print(dt.sample(n)) #n number of random data

dt.columns #print column name as lists
#dt.rows - not an attribute of dataframe

#printing a particular column:
#dt.('rows') - not callable
dt['rank']
...
dt[['rank' , 'salary']]     #multiple columns

dt['rank'].unique()  #unique - different objects of thst column

dt['gender'].unique()

dt['rank'].value_counts()  #value_counts() - counts number of particular objects in a column
dt['rank'].value_counts(normalize = True)

dt['gender'].unique()

dt['gender'].value_counts(normalize = True) #normalize gives result in fraction.
...
['BooleanDtype', 'Categorical', 'CategoricalDtype', 'CategoricalIndex',
 ...
 'IntervalIndex', 'MultiIndex', 'NA', 'NaT', 'NamedAgg', 'Period', 
 'PeriodDtype', 'PeriodIndex', 'RangeIndex', 'Series', 'SparseDtype', 
 'StringDtype', 'Timedelta', 'TimedeltaIndex', 'Timestamp', 'UInt16Dtype', 
 'UInt32Dtype', 'UInt64Dtype', 'UInt64Index', 'UInt8Dtype', '__builtins__', 
 '__cached__', '__doc__', '__docformat__', '__file__', '__getattr__', 
 '__git_version__', '__loader__', '__name__', '__package__', '__path__', 
 '__spec__', '__version__', '_config', '_hashtable', '_is_numpy_dev', '_lib', 
 '_libs', '_np_version_under1p17', '_np_version_under1p18', '_testing', 
 '_tslib', '_typing', '_version', 'api', 'array', 'arrays', 'bdate_range', 
 'compat', 'concat', 'core', 'crosstab', 'cut', 'date_range', 
 ...
 'read_stata', 'read_table', 'reset_option', 'set_eng_float_format', 
